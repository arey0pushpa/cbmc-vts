...
/****************************************
// BOOLEAN FUNCTION ON NODE AND SNARE-SNARE INHIBITION ON EDGE
// Variation_3 of the PLOS PAPER
*****************************************/
  ...
  for (i = 0; i < len; i++) {
    ...
    for (j = 0; j < snareLength; j++) {
      ...
      if ((v & (1 << j))) {             // Molecule is present
        if ((vf & (b1 << bv)) != b0) {  // Arg molecules were not able to
                                        // inhibit it i.e Molecule is  active
    ...
    for (k = 0; k < N; k++) {
      if (k != edgeBag[i].jth) {
        ...
        for (m = 0; m < snareLength; m++) {
          if (edgeBag[i].combinedMask &
              (1 << m)) {                // if molecule is in combined mask
          ...
          if (edgeBag[i].combinedMask2 &
              (1 << m)) {  // if molecule is in combined mask check for its
                           // absence
            if (Vnodes[k] & (1 << m)) {
              vf = nodeInhib[m + M];
              if ((vf & (b1 << bv)) != b0) {
